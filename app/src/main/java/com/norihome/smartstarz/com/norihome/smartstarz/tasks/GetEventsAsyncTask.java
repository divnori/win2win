...
public class GetEventsAsyncTask extends AsyncTask<Void, Void, List<EventBean>> {
    ...
    @Override
    public List<EventBean> doInBackground(Void... params) {
        System.out.println("------------          get events task called   --------------------");
        List<EventBean> eventBeanList = null;
        EventBeanCollection collection = null;
        SmartStarzApi.GetEvents getEvents = null;
        ...
        try {
            getEvents = myApiService.getEvents();
            collection = getEvents.execute();
            eventBeanList = collection.getItems();
        } catch (IOException e) {
            e.printStackTrace();
        }
        System.out.println("------------          returning from getevents   --------------------");
